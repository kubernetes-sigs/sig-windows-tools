...
Your Kubernetes server must be at or later than version 1.23 and your containerd at or later than version 1.6. To check the version, enter `kubectl version`.
...
# before applying it
curl -L https://raw.githubusercontent.com/kubernetes-sigs/sig-windows-tools/master/hostprocess/flannel/kube-proxy/kube-proxy.yml | sed 's/VERSION/v1.25.3/g' | kubectl apply -f -
...
Install Calico by getting the projectcalico release and creating the calico.yaml manifest.

```bash
wget https://github.com/projectcalico/calico/releases/download/$CALICO_VERSION/release-$CALICO_VERSION.tgz
# Extract the files
tar -xf release-$CALICO_VERSION.tgz
# Create the manifest
kubectl create -f release-$CALICO_VERSION/manifests/calico.yaml
```
...
```bash
curl -L https://raw.githubusercontent.com/kubernetes-sigs/sig-windows-tools/master/hostprocess/calico/kube-proxy/kube-proxy.yml | sed 's/VERSION/v1.25.3/g' | kubectl apply -f -
...
>  **Note** If you use, flannel hostprocess, before joining the node, copy the file from /run/flannel/subnet.env to your Windows machine to C:\run\flannel\subnet.env
